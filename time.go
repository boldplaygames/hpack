...
func (e *Encoder) encodeTime(tm time.Time) []byte {
	if e.timeBuf == nil {
		e.timeBuf = make([]byte, timeBufferMaxSize)
	}
...
func (d *Decoder) DecodeTime() (time.Time, error) {
	...
	if extID != timeExtID && extID != 13 {
		return time.Time{}, fmt.Errorf("hpack: invalid time ext id=%d", extID)
	}
...
func (d *Decoder) decodeTime(extLen int) (time.Time, error) {
	...
	default:
		err = fmt.Errorf("hpack: invalid ext len=%d decoding time", extLen)
		return time.Time{}, err
	}
}
