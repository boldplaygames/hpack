...
func makeExtEncoderAddr(extEncoder encoderFunc) encoderFunc {
	return func(e *Encoder, v reflect.Value) error {
		if !v.CanAddr() {
			return fmt.Errorf("hpack: EncodeExt(nonaddressable %T)", v.Interface())
		}
...
func makeExtDecoder(
	...
	return nilAwareDecoder(typ, func(d *Decoder, v reflect.Value) error {
		...
		if extID != wantedExtID {
			return fmt.Errorf("hpack: got ext type=%d, wanted %d", extID, wantedExtID)
		}
...
func makeExtDecoderAddr(extDecoder decoderFunc) decoderFunc {
	return func(d *Decoder, v reflect.Value) error {
		if !v.CanAddr() {
			return fmt.Errorf("hpack: DecodeExt(nonaddressable %T)", v.Interface())
		}
...
func (d *Decoder) parseExtLen(c byte) (int, error) {
	...
	case msgpcode.Ext32:
		...
		return int(n), err
	default:
		return 0, fmt.Errorf("hpack: invalid code=%x decoding ext len", c)
	}
}
...
func (d *Decoder) decodeInterfaceExt(c byte) (interface{}, error) {
	...
	if !ok {
		return nil, fmt.Errorf("hpack: unknown ext id=%d", extID)
	}
